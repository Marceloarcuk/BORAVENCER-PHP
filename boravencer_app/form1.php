...
if(isset($_REQUEST['facebook']) && $_REQUEST['facebook']==true ){
		$sql=" select id_facebook, id_cri, email_cri, nome_cri, cpf_cri,pass_cri from tb_crianca  where email_cri='".tStr($_POST['email'])."';";
        $consult=$conection->prepare($sql1);
        $dados=($consulta->execute()) ? $consulta->fetch(PDO::FETCH_OBJ) : false;
        if($dados != false){
			 $idface = $dados->id_facebook;
			$idpass1  = $dados->pass_cri;
			if($idpass != ''){
				echo "<script>alert('O usuário não está cadastrado com o Facebook!'); </script>";
...
if(isset($_REQUEST['validar']) && $_REQUEST['validar']==true){
	...
	if (isset($_POST['email'])){
		$_SESSION['emailuser']=$_POST['email'];
     	...
     	$sql1=" select id_facebook, id_cri, email_cri, nome_cri, cpf_cri,pass_cri from tb_crianca  where email_cri='".tStr($_POST['email'])."';";
        $consulta1=$conection->prepare($sql1);
        $dados1=($consulta1->execute()) ? $consulta1->fetch(PDO::FETCH_OBJ) : false;
        if($dados1 != false){
			
            $idface1 = $dados1->id_facebook;
			$idpass  = $dados1->pass_cri;
                                ...
                                <div class="row">
                                    ...
                                    <div class="row text-center">
                                        <form action="?facebook=true" method="post" >
                                            <br> <a href="<?php if(isset($loginUrl)){ echo $loginUrl; }  ?>"><img src="imagens/facebook-login-button.png" > </a>
